...
interface BaseParams<T extends 'zoom' | 'slide'> {
  type: T;
...
export type AnimationType = ZoomParams | SlideParams;
...
export class AnimatedCube {
  ...
  constructor(value: number) {
    ...
    inner.className = `z-1 flex h-full w-full items-center justify-center rounded text-xl font-bold shadow ${value ? CUBE_CLASS_MAP[value] : 'opacity-0'}`;
  ...
  zoom(params: Omit<ZoomParams, 'type' | 'target'>) {
    ...
    this.#target.className += ` ${CUBE_CLASS_MAP[value]}`;
